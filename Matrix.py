# Взять класс Матрица (написанный ранее) и написать для него по 3-4 теста каждого вида (PyTest, UnitTest, DocTest)
import doctest
...
class Matrix():
    ...
    def __mul__(self, other):
        ...
        new_res_matrix = []
        if len(self.res_matrix) == len(new_other_matrix.res_matrix) and len(self.res_matrix[0]) == len(
                new_other_matrix.res_matrix[0]):
            for i in range(len(self.res_matrix)):
                raw = []
                for j in range(len(self.res_matrix)):
                    number = sum(
                        [l for l in map(lambda x, y: x * y, self.res_matrix[i], new_other_matrix.res_matrix[j])])
                    raw.append(number)
